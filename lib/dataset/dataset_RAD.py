...
class VQARADFeatureDataset(Dataset):
    def __init__(self, name, cfg, dictionary, dataroot='data'):
        ...
        for sample in self.entries:
            ...
            img="<image>\n"
            en=get_conv(sample['qid']+start_index,sample['image_name'],img+sample['question']+label,sample['answer_text'])
            data.append(en)

        with open('/home/tj/QCR_PubMedCLIPs_fusion/rad_train_pro.json', 'w', encoding='utf-8') as json_file:
        ...
        # print(self.ab_target_open)
        # print(self.chest_target_open)
        # print(self.head_target_open)
        # print(self.ab_target_close)
        
        # print(self.head_target_close)
        
        # print(self.chest_target_close)
        ...
        for entry in self.entries:
            ...
            if None!=answer:
                ...
                if len(labels):
                    ...
                    if entry['image_organ']=="ABD":
                        if entry['answer_type']=='CLOSED':
                            self.ab_target_close.scatter_(0, labels, scores)
                        else:
                            self.ab_target_open.scatter_(0, labels, scores)           
                    elif entry['image_organ']=="HEAD":
                        if entry['answer_type']=='CLOSED':
                            self.head_target_close.scatter_(0, labels, scores)
                        else:
                            self.head_target_open.scatter_(0, labels, scores)
                    elif entry['image_organ']=="CHEST":
                        if entry['answer_type']=='CLOSED':
                            self.chest_target_close.scatter_(0,labels, scores)
                        else:
                            self.chest_target_open.scatter_(0,labels, scores)
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.TRAIN.VISION.CLIP:
            if self.cfg.TRAIN.VISION.CLIP_VISION_ENCODER == "RN50x4":
                ...
                clip_images_data = self.clip_images_data[entry['image']].reshape(3*self.img_size*self.img_size)
